    ...
    try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (fileTasks.length > 0) {
              ...
              console.log(
                `📝 ${absolutePath} → ${fileTasks.length} task${fileTasks.length > 1 ? 's' : ''}`
              );
            }
      ...
      if (allTasks.length > 0) {
        ...
        if (frontmatterTasks.length > 0) {
          console.log(
            `   └─ 🖼️  Frontmatter coverImage tasks: ${frontmatterTasks.length}`
          );
        }
        ...
        console.log(
          `🎨 Images per task: ${mergedOptions.numImages} variations`
        );
        console.log(
          `📈 Total images to generate: ${allTasks.length * mergedOptions.numImages!}`
        );
