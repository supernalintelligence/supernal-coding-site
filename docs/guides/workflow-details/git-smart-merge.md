...
## Agent Commits (`[SC]` Tagging)

When AI agents make commits through `sc git commit --auto`, commits are:

1. **Tagged with `[SC]` prefix** - Immediately visible in `git log`
2. **Unsigned** - Human commits are GPG-signed, agent commits are not

This provides clear provenance for AI-generated commits:

```bash
# Example git log showing distinction
abc1234 [SC] chore: auto-commit before sync-push    # Agent commit (unsigned)
def5678 feat: implement auth flow                    # Human commit (signed)
```

### Checking Commit Source

```bash
# Show signed vs unsigned commits
git log --show-signature -5

# Filter for agent commits
git log --oneline --grep="\[SC\]"

# Filter for human commits (signed)
git log --oneline --show-signature | grep "Good signature"
```

## Related Documentation

- **[Git Commands Reference](../../cli-commands/git-commands.md)** - Full CLI reference for `sc git` and `sc git-smart`
- **[Git Hooks Configuration](../git-hooks-configuration.md)** - Pre-commit and pre-push hooks
- **Requirements System** - Requirement tracking integration (see project CLI documentation)

---

_Last Updated: 2025-12-08_  
_Related: EPIC-002 (Git Safety & Smart Workflow), REQ-INFRA-111 (Agent Commit Signing)_
